...
# Schema for Structured Entity Extraction from natural language chat history
...
# Reference Metadata for Frontend UI rendering (matching exact original fields)
FLOW_METADATA = {
    "facilitator": {
        "id": "facilitator",
...
# Session structure: { 'answers': dict, 'completed': bool, 'messages': list }
sessions = {}

def get_missing_fields(answers):
    """
    Evaluates the currently missing fields dynamically based on the state of answers
    ...
    if "challenges" not in answers or answers["challenges"] is None:
        ...
        # If challenges contains 'Other', challenges_other is required
...
If the user chooses a pill/choice option, extract that exact value.
If the user explicitly states they had no challenges or type 'None', set challenges to [].
...
We are collecting data in a simulated WhatsApp chat interface. Your goal is to guide the user naturally through filling out their report.
...
4. Keep your message relatively concise and formatting clean (WhatsApp style). Use bold text (**like this**) for emphasis.
...
def run_extraction(messages_history, current_answers):
    """Executes Structured Entity Extraction using Gemini and updates state."""
    try:
        # Create LLM with structured output mapping to ReportForm model
        ...
        # Use last 12 messages to keep context short and clear
        ...
        # Merge new non-None extracted values into our current state
        updates = extracted_data.model_dump(exclude_unset=True, exclude_none=True)
        for k, v in updates.items():
            if v is not None:
                current_answers[k] = v
                
        # --- Clean up conditional logic branches dynamically ---
        
        # If they did not meet this month, remove any met-specific values
        ...
        # If they don't have 'Other' in challenges, remove challenges_other
        challenges = current_answers.get("challenges", [])
        if isinstance(challenges, list) and "Other" not in challenges:
            current_answers.pop("challenges_other", None)
            
        # If they don't want to add a testimony, remove testimony details
        ...
        # If testimony affirmations are not verbalized, remove details
...
def generate_bot_response(messages_history, current_answers, next_field_info):
    """Generates a warm, conversational follow-up prompt using the conversational LLM."""
    try:
        ...
        return f"I've noted that. Let's move on: {next_field_info['question']}"
...
def index():
    try:
        with open("index.html", "r", encoding="utf-8") as f:
            html_content = f.read()
        # We replace the title/subheading slightly in index.html to represent the new Langchain status
        html_content = html_content.replace(
            "Rule-Based WhatsApp Conversational Flow Prototype", 
            "LangChain-Powered Conversational AI Agent"
        )
...
def reset_session():
    ...
    # Serve the first field metadata to render the frontend pills
...
def process_message():
    ...
    if not os.environ.get("GEMINI_API_KEY"):
        return jsonify({
            "message": "⚠️ **GEMINI_API_KEY Missing**\n\nPlease save your `GEMINI_API_KEY` in the `.env` file in your project folder, then restart your Flask application.",
            "completed": False,
            "state": {"step_index": 0, "answers": {}, "completed": False}
        })
    ...
    # 1. Save user's message to conversation log
    session["messages"].append({"role": "user", "content": user_message})
    
    # 2. Run LangChain Structured Extraction to update answers state from conversation context
    run_extraction(session["messages"], session["answers"])
    
    # 3. Assess which fields are still missing
    missing_fields = get_missing_fields(session["answers"])
    
    # Compute progress index based on filled fields
    total_fields = len(FLOW_METADATA)
    filled_count = total_fields - len(missing_fields)
    
    if not missing_fields:
        # All fields are successfully captured! Generate the report
    ...
    # 4. We still have missing fields. Find the next target field
    next_field_id = missing_fields[0]
    next_meta = FLOW_METADATA[next_field_id]
    
    # Format a step object for index.html UI state mapping
    step_data = {
        "id": next_field_id,
    ...
    # 5. Run LangChain Conversational LLM to generate a natural, friendly reply seeking the field
    bot_reply = generate_bot_response(session["messages"], session["answers"], step_data)
    
    # 6. Save bot's reply and return to client
